...
fun percentUsingLanguage(programeri: List<Programer>, lang: String): Double {
    var brojJezika: Double = 0.0
    var ukupnoProgramera: Double = 0.0
    val jezik = lang.lowercase()
    for (p in programeri) {
        ukupnoProgramera++
        for (j in p.jeziciNorm) {
            if (j==jezik) 
    ...
    return brojJezika/ukupnoProgramera * 100
}
fun main() {
